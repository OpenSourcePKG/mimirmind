...
//
// hip_attention_prefill_flash_q8_0_gqa_probe — parity check for the
// GQA-Head-Packed variant of the Q8_0-KV prefill kernel. Output shape
// is identical to the plain Q8_0-prefill kernel; only the launch
// geometry differs (grid = (nKvHeads, T_q, 1) instead of
// (nHeads, T_q, 1)). CPU reference is the same monolithic attention
// as the plain probe — this validates that packing all Q-heads of a
// KV-group into one workgroup produces bit-identical output.
//
// Test-shape has nHeads=8, nKvHeads=2 → nQPerKv=4 (well within the
// compile-time cap N_Q_PER_KV_MAX=8, exercises the "some slots idle"
// path in the unrolled loops).
...
constexpr int          kNBlocksPerRow  = kKvDim / kBlockElems;
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco"
          / "attention_prefill_flash_q8_0_gqa.hsaco").string();
}
...
void monolithicAttentionQ8_0CpuRef(
                  ...
                  void* devQ, void* devK, void* devV, void* devCurLen)
{
    ...
    // Grid: (nKvHeads, T_q, 1) — the head-packing rearranges over
    // KV-heads instead of Q-heads. Output layout is unchanged.
    kernel.launch(stream,
                  /*grid=*/  static_cast<std::uint32_t>(kNumKvHeads),
    ...
    monolithicAttentionQ8_0CpuRef(hostQ, hostK, hostV, ref,
    ...
    if (badIdx != SIZE_MAX && maxRatio > 1e-3f) {
        const std::size_t pq = badIdx / kQStride;
        const std::size_t rem = badIdx % kQStride;
        const std::size_t hq  = rem / kHeadDim;
        const std::size_t d   = rem % kHeadDim;
        std::printf("    worst @ pq=%zu hq=%zu d=%zu: gpu=%.6g cpu=%.6g\n",
                    pq, hq, d,
...
int main(int argc, char** argv) {
    ...
    const int nQPerKv = kNumHeads / kNumKvHeads;
    std::printf("hip_attention_prefill_flash_q8_0_gqa_probe:\n  hsaco: %s\n"
                "  T_q=%d nHeads=%d nKvHeads=%d nQPerKv=%d headDim=%d curLen=%d block=%u\n"
                "  grid=(nKvHeads=%d, T_q=%d, 1)\n",
                hsacoPath.c_str(),
                kT_q, kNumHeads, kNumKvHeads, nQPerKv, kHeadDim, kCurLen, kBlock,
                kNumKvHeads, kT_q);
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("attention_prefill_flash_q8_0_gqa");
        ...
        fillRandomFloat(hostQ, /*seed=*/0xC0AC1001u, /*scale=*/1.0f);
        fillRandomQ8_0 (hostK, /*seed=*/0xC0AC1002u);
        fillRandomQ8_0 (hostV, /*seed=*/0xC0AC1003u);
        ...
        std::printf("\nhip_attention_prefill_flash_q8_0_gqa_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_attention_prefill_flash_q8_0_gqa_probe: threw: %s\n", e.what());
        return 2;
    }
}