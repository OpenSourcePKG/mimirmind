...
//
// hip_attention_probe — parity check for the untiled multi-head
// attention kernel (M5f.3 variant a).
//
// Test shape: T_q=4, curLen=32, nHeads=8, nKvHeads=2, headDim=128.
// The T_q=4 setup exercises the 2D grid geometry (nHeads × T_q) and
// the causal-mask offsetting across query positions.
//
// Two cases share the same random Q/K/V:
//   A: slidingWindow=0    — full causal, 33..36 keys per query.
//   B: slidingWindow=16   — SWA-truncated to 16 keys per query.
//
// Combined-tolerance 1e-4/1e-4. Attention has headDim-term dot +
// softmax + K-term V-weighted sum → drift ~ULP for these shapes.
...
constexpr int          kCurLen     = 32;                            // positionOffset
constexpr int          kCacheSlots = 48;                            // curLen + T_q + safety
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco" / "attention.hsaco").string();
}
                  ...
                  void* devQ, void* devK, void* devV, void* devCurLen)
{
    ...
    attentionCpuRef(hostQ, hostK, hostV, ref,
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_attention_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("attention");
        ...
        fillRandom(hostQ, /*seed=*/0xA771A001u, /*scale=*/1.0f);
        fillRandom(hostK, /*seed=*/0xA771A002u, /*scale=*/1.0f);
        fillRandom(hostV, /*seed=*/0xA771A003u, /*scale=*/1.0f);
        ...
        std::printf("\nhip_attention_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_attention_probe: threw: %s\n", e.what());
        return 2;
    }
}