...
//
// hip_attention_fp16_probe — parity check for the fp16-KV variant
// of the untiled multi-head attention kernel.
//
// Same 2D-grid geometry and 2-case pattern as the f32 probe. K and V
// are fp16 in device memory; the CPU reference promotes via
// __half2float in the exact same spots the kernel does, so drift
// stays at ULP level in fp32-space.
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco" / "attention_fp16.hsaco").string();
}
                  ...
                  void* devQ, void* devK, void* devV, void* devCurLen)
{
    ...
    attentionFp16CpuRef(hostQ, hostK, hostV, ref,
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_attention_fp16_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("attention_fp16");
        ...
        fillRandomFloat(hostQ, /*seed=*/0xA771A001u, /*scale=*/1.0f);
        fillRandomHalf (hostK, /*seed=*/0xA771A002u, /*scale=*/1.0f);
        fillRandomHalf (hostV, /*seed=*/0xA771A003u, /*scale=*/1.0f);
        ...
        std::printf("\nhip_attention_fp16_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_attention_fp16_probe: threw: %s\n", e.what());
        return 2;
    }
}