...
//
// hip_attention_prefill_flash_q8_0_probe — Q8_0-KV variant of the
// prefill-flash probe. K/V are ggml-style Q8_0 blocks (2 B fp16
// scale + 32 B int8 quants). CPU reference dequants the exact same
// bytes the kernel does.
//
// Same random-bytes-directly strategy as the decode Q8_0 probes:
// no fp32→Q8_0 round-trip, both sides see identical Q8_0 blob.
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco" / "attention_prefill_flash_q8_0.hsaco").string();
}
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_attention_prefill_flash_q8_0_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("attention_prefill_flash_q8_0");
        ...
        fillRandomFloat(hostQ, /*seed=*/0xE8B01001u, /*scale=*/1.0f);
        fillRandomQ8_0 (hostK, /*seed=*/0xE8B01002u);
        fillRandomQ8_0 (hostV, /*seed=*/0xE8B01003u);
        ...
        std::printf("\nhip_attention_prefill_flash_q8_0_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_attention_prefill_flash_q8_0_probe: threw: %s\n", e.what());
        return 2;
    }
}