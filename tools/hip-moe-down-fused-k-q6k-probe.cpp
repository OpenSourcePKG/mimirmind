...
//
// hip_moe_down_fused_k_q6k_probe — parity check for the Q6_K fused MoE
// down-projection over k active experts. Mirrors
// hip_moe_down_fused_k_q8_0_probe with a Q6_K super-block generator +
// CPU reference dequant that matches compute::quant::Q6K::dequantToF32.
//
// The accum buffer is read-modify-write; the probe pre-seeds it and
// checks the GPU output equals seed + computed_delta (not delta alone).
...
// Q6_K super-block: 128 B ql + 64 B qh + 16 B sc + 2 B d = 210 B.
...
constexpr int          kNBlocksPerRow  = kFfPer / kBlockElems;         // 4
constexpr int          kRowBytes       = kNBlocksPerRow * kBlockBytes; // 840
constexpr int          kExpertBytes    = kDModel * kRowBytes;          // ~105 KB
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco" / "moe_down_fused_k_q6k.hsaco").string();
}
...
// Build a stack of `nExperts` Q6_K weight banks with random content.
// Each super-block: random d (fp16, positive small), random ql (0..15
// nibbles per byte, 128 B = 256 nibbles → 128 pairs / 256 low+high),
// random qh (0..255 = 4 × 2-bit pairs per byte), signed random sc
// (16 int8 scales).
void fillRandomQ6_KBank(std::vector<std::uint8_t>& bytes,
    ...
    for (int e = 0; e < nExperts; ++e) {
        ...
        for (int r = 0; r < kDModel; ++r) {
            ...
            for (int b = 0; b < kNBlocksPerRow; ++b) {
                ...
                // ql — 128 B of packed 4-bit nibbles.
                for (int i = 0; i < 128; ++i) {
                    x = x * 1664525u + 1013904223u;
                    blk[i] = static_cast<std::uint8_t>(x >> 24);
                }
                // qh — 64 B, each byte holds four 2-bit pairs.
                for (int i = 0; i < 64; ++i) {
                    x = x * 1664525u + 1013904223u;
                    blk[128 + i] = static_cast<std::uint8_t>(x >> 24);
                }
                // sc — 16 B of int8 scales (-16..15-ish range keeps
                // the accumulator sane after ×d×ql).
                for (int i = 0; i < 16; ++i) {
                    x = x * 1664525u + 1013904223u;
                    blk[192 + i] = static_cast<std::uint8_t>(
                        (static_cast<std::int32_t>(x >> 24) % 32) - 16);
                }
                // d — fp16 positive small scalar.
                x = x * 1664525u + 1013904223u;
                const float dScalar =
                    0.001f + 0.02f * (static_cast<float>(x & 0xFFFF) / 65535.0f);
                const __half dHalf = __float2half(dScalar);
                std::memcpy(blk + 208, &dHalf, sizeof(__half));
            }
        }
    }
}

// Single-element dequant matching compute::quant::Q6K::dequantToF32:
// col ∈ [0..256), split at 128, per-half l ∈ [0..32) with 4 quadrants.
inline float dequantElemQ6K(const std::uint8_t* rowBase, int col) {
    const int b     = col / kBlockElems;
    const int inSb  = col % kBlockElems;
    const int half  = inSb / 128;
    const int rem   = inSb % 128;
    const int quad  = rem / 32;                       // 0..3
    const int l     = rem % 32;                       // 0..31

    const std::uint8_t* blk = rowBase + static_cast<std::size_t>(b) * kBlockBytes;
    const std::uint8_t* ql  = blk + half * 64;        // 64 B per half
    const std::uint8_t* qh  = blk + 128 + half * 32;  // 32 B per half
    const auto*         sc  = reinterpret_cast<const std::int8_t*>(blk + 192)
                              + half * 8;             // 8 B per half
    __half hScale;
    std::memcpy(&hScale, blk + 208, sizeof(__half));
    ...
    switch (quad) {
        case 0:  // l +  0
            q  = static_cast<int>((ql[l +  0] & 0x0Fu) |
                                  (((qh[l] >> 0) & 0x03u) << 4)) - 32;
            si = is + 0;
            break;
        case 1:  // l + 32
            q  = static_cast<int>((ql[l + 32] & 0x0Fu) |
                                  (((qh[l] >> 2) & 0x03u) << 4)) - 32;
            si = is + 2;
            break;
        case 2:  // l + 64 (= ql[l+0] >> 4)
            q  = static_cast<int>((ql[l +  0] >> 4u) |
                                  (((qh[l] >> 4) & 0x03u) << 4)) - 32;
            si = is + 4;
            break;
        case 3:  // l + 96 (= ql[l+32] >> 4)
            q  = static_cast<int>((ql[l + 32] >> 4u) |
                                  (((qh[l] >> 6) & 0x03u) << 4)) - 32;
            si = is + 6;
            break;
    }
    return d * static_cast<float>(sc[si]) * static_cast<float>(q);
}
        ...
        for (int k = 0; k < K; ++k) {
            ...
            for (int l = 0; l < ffPer; ++l) {
                innerAcc += static_cast<double>(Xk[l])
                          * static_cast<double>(dequantElemQ6K(row, l));
            }
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_moe_down_fused_k_q6k_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("moe_down_fused_k_q6k");
        ...
        fillRandomFloat   (hostX,          /*seed=*/0x60E0001u, /*scale=*/0.5f);
        fillRandomQ6_KBank(hostW,          /*seed=*/0x60E0002u, kNExperts);
        fillRandomFloat   (hostKw,         /*seed=*/0x60E0003u, /*scale=*/1.0f);
        fillRandomFloat   (hostAccumSeed,  /*seed=*/0x60E0004u, /*scale=*/1.0f);

        // Random expert indices in [0, nExperts).
        {
            std::uint32_t x = 0x60E0005u;
            for (int i = 0; i < kK; ++i) {
                x = x * 1664525u + 1013904223u;
        ...
        // Q6_K terms are ~40× larger magnitude than Q8_0's int8 range
        // and Kahan-compensated in the kernel — same tolerance as the
        // matmul_q6k_vec probe on gfx1101.
        ...
        std::printf("\nhip_moe_down_fused_k_q6k_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_moe_down_fused_k_q6k_probe: threw: %s\n", e.what());
        return 2;
    }
}
