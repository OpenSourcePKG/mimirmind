...
//
// hip_matmul_q8_gemm_probe — parity check for the M-way generalisation
// of matmul_q8_0_vec. Same Q8_0 dequant path, but processes M_TILE=8
// token rows per workgroup with the 4-outputs-per-WG sub-group pattern.
//
// Test-shape M=8 exercises the full M_TILE without padding; N=128
// exercises the 4-outputs-per-WG stripe; K=1024 is one full X-tile.
//
// Uses the same random-Q8_0-bytes strategy as hip_matmul_q8_probe:
// CPU reference dequantises the exact same bytes (via __float2half /
// __half2float roundtrip for the scale) so ULP-level fp32-reduction
// drift is the only expected diff.
...
// Native Q8_0 row layout: nBlocks blocks of 34 B each (fp16 scale +
// 32 int8 quants, interleaved).
...
// Dequantise element (n, k) from the native-layout Q8_0 blob.
...
// CPU reference — fp64 GEMM using dequantised bytes.
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_matmul_q8_gemm_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("matmul_q8_0_gemm");
        ...
        fillRandomFloat(hostX, /*seed=*/0x6E110001u, /*scale=*/0.5f);
        fillRandomQ8_0Native(hostW, /*seed=*/0x6E110002u, /*nRows=*/kN);
        ...
        std::printf("\nhip_matmul_q8_gemm_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_matmul_q8_gemm_probe: threw: %s\n", e.what());
        return 2;
    }
}