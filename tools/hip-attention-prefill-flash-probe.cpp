...
//
// hip_attention_prefill_flash_probe — parity check for the streaming
// online-softmax FlashAttention prefill kernel (fp32 KV).
//
// Test-shape uses T_q=6 to exercise the multi-query grid geometry
// and the causal-offset variation across query positions. Kernel
// output is compared against a MONOLITHIC (no-tiling) attention fp64
// CPU reference — the streaming Dao-2022 identity must reproduce
// this ULP-close.
//
// Two cases:
//   A: slidingWindow=0    — full causal, ~65..70 keys per query
//   B: slidingWindow=32   — SWA, tile-boundary crossing exercised
...
constexpr std::uint32_t kBlock     = 16;   // ATTN_FLASH_PREFILL_LOCAL
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco" / "attention_prefill_flash.hsaco").string();
}
        ...
        for (int hq = 0; hq < nHeads; ++hq) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                for (int d = 0; d < headDim; ++d) {
                    acc += static_cast<double>(qVec[d]) * static_cast<double>(kVec[d]);
                }
                  ...
                  void* devQ, void* devK, void* devV, void* devCurLen)
{
    ...
    monolithicAttentionCpuRef(hostQ, hostK, hostV, ref,
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_attention_prefill_flash_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("attention_prefill_flash");
        ...
        fillRandom(hostQ, /*seed=*/0xF0EA1001u, /*scale=*/1.0f);
        fillRandom(hostK, /*seed=*/0xF0EA1002u, /*scale=*/1.0f);
        fillRandom(hostV, /*seed=*/0xF0EA1003u, /*scale=*/1.0f);
        ...
        std::printf("\nhip_attention_prefill_flash_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_attention_prefill_flash_probe: threw: %s\n", e.what());
        return 2;
    }
}