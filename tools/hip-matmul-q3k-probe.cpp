...
//
// hip_matmul_q3k_probe — parity check for matmul_q3k_vec. Loads
// matmul_q3k_vec.hsaco, runs it on synthetic Q3_K-quantized weights +
// random X, compares against an inline CPU reference that dequantizes
// the same block via the kmask1/kmask2 12-byte scales trick and does
// the dot product in double precision.
//
// Test shape: N=128 outputs × K=1024 elements (4 Q3_K super-blocks per
// row). Small enough to run fast, exercises the tiling path
// (X_TILE_ELEMENTS=1024 so one full tile pass), and small enough that
// the fp64 CPU reference is trivial.
//
// We do NOT round-trip real floats through Q3_K packing. Instead we
// generate random block scales + hmask + qs bytes directly and give
// both the GPU (packed bytes) and the CPU (dequant to fp32) exactly
// the same data. That way any diff is a matmul bug, not packing noise.
//
// Uses the combined-tolerance pattern (|diff| <= abs_tol + rel_tol * |ref|).
// Q3_K dequant + 1024-term parallel FMA has similar noise to Q6_K, so
// tol is looser (1e-3 abs + 1e-3 rel) than pure-fp32 element-wise probes.
...
constexpr int          kBlocksPerRow     = kK / kBlockElems;  // 4
constexpr std::uint32_t kThreadsPerBlock = 128;               // MATMUL_Q3K_LOCAL
...
std::string defaultHsacoPath(const char* argv0) {
    ...
    return (exe.parent_path() / "hsaco" / "matmul_q3k_vec.hsaco").string();
}
...
// Unpack the 12 packed scale bytes into 16 6-bit unsigned scales.
// Bit-identical to src/compute/quant/Q3K.cpp::unpackScales — this is
// the CPU-side check that the GPU-side unpackScalesQ3K matches.
void unpackScalesQ3K(const std::uint8_t (&packed)[12],
                     std::uint8_t (&outScales)[16])
{
...
// Build a random Q3_K weight blob (N rows × K/256 blocks × 110 bytes).
// Also returns the dequantized fp32 weight matrix for the CPU
// reference — same numbers on both sides. Round-trips d through fp16
// so the CPU reference sees the exact scale value the GPU kernel reads
// (skipping this leaks ~10 extra bits of precision and creates
// false-positive FAILs on 1024-term dot products).
void makeRandomQ3K(std::vector<std::uint8_t>& packed,
                   std::vector<float>&        dequant,
                   std::uint32_t              seed)
{
    const std::size_t rowBytes =
        static_cast<std::size_t>(kBlocksPerRow) * kBlockBytes;
    ...
    for (int n = 0; n < kN; ++n) {
        ...
        for (int b = 0; b < kBlocksPerRow; ++b) {
            ...
            // hmask (32 B) — random high bits.
            for (int i = 0; i < 32; ++i) {
                blk[i] = static_cast<std::uint8_t>(nextRand(s) >> 24);
            }
            // qs (64 B) — random low 2 bits per element.
            for (int i = 0; i < 64; ++i) {
                blk[32 + i] = static_cast<std::uint8_t>(nextRand(s) >> 24);
            }
            // scales packed (12 B) — random.
            std::uint8_t packedScales[12];
            for (int i = 0; i < 12; ++i) {
                packedScales[i] = static_cast<std::uint8_t>(nextRand(s) >> 24);
                blk[96 + i]     = packedScales[i];
            }
            // d — fp16 positive small scalar, keeps sums in a stable range.
            const std::uint32_t r = nextRand(s);
            const float dScalar =
                0.001f + 0.02f * (static_cast<float>(r & 0xFFFF) / 65535.0f);
            const __half dHalf = __float2half(dScalar);
            std::memcpy(blk + 108, &dHalf, 2);

            // Round-trip d through fp16 for the CPU reference.
            const float dFromHalf = __half2float(dHalf);

            // Dequant this block into `dequant[n, b*256 .. b*256+255]`
            // using the exact loop shape of Q3K::dequantToF32.
            std::uint8_t scales[16];
            unpackScalesQ3K(packedScales, scales);

            const std::uint8_t* hmask = blk;
            const std::uint8_t* qs    = blk + 32;

            float* outBase = dequant.data()
                           + static_cast<std::size_t>(n) * kK
                           + static_cast<std::size_t>(b) * kBlockElems;

            int is    = 0;
            int mBit  = 0;
            const std::uint8_t* q = qs;
            float* dst = outBase;
            for (int nHalf = 0; nHalf < 256; nHalf += 128) {
                (void)nHalf;
                int shift = 0;
                for (int j = 0; j < 4; ++j) {
                    const float dl0 = dFromHalf
                                    * (static_cast<int>(scales[is++]) - 32);
                    for (int l = 0; l < 16; ++l) {
...
int main(int argc, char** argv) {
    ...
    std::printf("hip_matmul_q3k_probe:\n  hsaco: %s\n"
    ...
    try {
        ...
        HipKernel kernel = mod.getKernel("matmul_q3k_vec");
        ...
        fillFloats  (hostX,          /*seed=*/0x30E0A5A5u, /*scale=*/0.5f);
        makeRandomQ3K(hostW, hostWDequant, /*seed=*/0x30E05A5Au);
        ...
        std::printf("\nhip_matmul_q3k_probe: %s\n", ok ? "OK" : "FAIL");
        return ok ? 0 : 1;

    } catch (const std::exception& e) {
        std::fprintf(stderr, "hip_matmul_q3k_probe: threw: %s\n", e.what());
        return 2;
    }
}
