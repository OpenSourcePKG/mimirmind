 ...
 * through `CudaModule` + `CudaKernel` + `CudaStream` instead of L0
 * modules / command queues.
 *
 * Same class name (`GpuOps`) as the L0 side; disambiguation happens
 * through the `compute::cuda::` namespace vs `compute::l0::`. This
 * mirrors the `src/core/gpu/{l0,hip}/` primitive layout.
 *
 * Skeleton stage (Schritt 3b sub-A): the ctor loads every `.ptx`
...
public:
    ...
    void deltanetChunkForwardAsync(const float* q, const float* k, const float* v,
                                   const float* gCum, const float* beta,
    ...
    // goes through cudaMemcpyAsync on the shared stream; flush
    // syncs the stream.
    ...
    /// this is a plain `cudaMalloc`'d int; the host must use
    /// `cudaMemcpy(...H2D)` to update it (no zero-copy USM equivalent
    /// on discrete gfx1101).
