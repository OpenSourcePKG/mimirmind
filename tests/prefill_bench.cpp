...
struct Args {
    ...
    // Optional: derive T_k from the byte count of a raw prompt file
    // (chars/kCharsPerToken). No tokenizer dep — the ratio is a
    // conservative approximation shared with samples/generate.py so
...
void printHelp() {
    std::cout <<
        ...
        "  --prompt-file PATH\n"
        "                derive --tk from file size / --cpt (overrides --tk).\n"
        "                Accepts raw .txt or a needle .json (uses .prompt).\n"
        "                Label is appended with the file basename.\n"
        "  --cpt F       chars/token ratio for --prompt-file (default 4.0,\n"
        "                matches samples/generate.py for English; use 3.0 for de)\n"
...
Args parseArgs(int argc, char** argv) {
    ...
    if (!a.promptFile.empty()) {
        std::ifstream f(a.promptFile, std::ios::binary);
        if (!f) {
            std::cerr << "prefill_bench: cannot open --prompt-file '"
                      << a.promptFile << "'\n";
            std::exit(2);
        }
        std::stringstream ss;
        ...
        // If the file looks like a needle JSON ({"prompt": "..."}),
        // approximate the .prompt payload length by finding its slice
        // between the first "prompt":" and the matching close-quote.
        // Small hand-parse avoids pulling in a JSON dep — the file is
        // machine-generated by samples/generate.py.
        const std::string key = "\"prompt\":";
        const auto pk = body.find(key);
        if (pk != std::string::npos) {
            auto qs = body.find('"', pk + key.size());
            if (qs != std::string::npos) {
                // Advance past the opening quote and walk until an
                ...
                if (i < body.size()) {
                    body = body.substr(start, i - start);
                }
            }
        }

        if (a.charsPerToken <= 0.0) {
            std::cerr << "prefill_bench: --cpt must be > 0\n";
            std::exit(2);
        }
        const std::size_t derived = static_cast<std::size_t>(
            std::llround(static_cast<double>(body.size())
                         / a.charsPerToken));
        a.tk = std::max<std::size_t>(a.tq, derived);
        const std::string base =
            std::filesystem::path(a.promptFile).stem().string();
        a.label += "__" + base + "_tk" + std::to_string(a.tk);
    }
