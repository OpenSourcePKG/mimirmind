// SPDX-License-Identifier: Apache-2.0
// Copyright 2026 Stefan Werfling
//
// FP16 variant of rope_mrope (interleaved multi-axis RoPE, in-place) — the
// FP16-KV IMRoPE piece that was the load-bearing gap for a 16-bit KV cache on
// Qwen3-Next / Qwen3.5-MoE full-attention layers. Byte-for-byte the same index
// math, section rule and rotation as rope_mrope.cu; only the operand storage is
// __half. Like rope_inplace_fp16, the rotation is computed in fp32 registers
// (__half2float -> rotate -> __float2half) so RoPE precision does not degrade
// from the 16-bit storage.
//
// Launch (identical to rope_mrope):
//   dim3 grid ( ceil(seqLen*numHeads*halfDim / ROPE_LOCAL), 1, 1 )
//   dim3 block( ROPE_LOCAL, 1, 1 )

#include <cuda_runtime.h>
#include <cuda_fp16.h>

#ifndef ROPE_LOCAL
#define ROPE_LOCAL 256
#endif

extern "C" __global__ __launch_bounds__(ROPE_LOCAL)
void rope_mrope_fp16(
    __half*      __restrict__ x_base,
    const int                 seqLen,
    const int                 numHeads,
    const int                 headDim,
    const int*   __restrict__ startPosPtr,
    const float               base,
    const int                 writeOffsetStride,
    const int                 sec0,
    const int                 sec1,
    const int                 sec2,
    const int                 sec3)
{
    const int gid     = blockIdx.x * blockDim.x + threadIdx.x;
    const int halfDim = headDim / 2;
    const int total   = seqLen * numHeads * halfDim;
    if (gid >= total) {
        return;
    }

    const int i  = gid % halfDim;
    const int hp = gid / halfDim;
    const int h  = hp % numHeads;
    const int p  = hp / numHeads;

    const int   startPos = startPosPtr[0];
    const float pos      = (float)(startPos + p);
    const float posAxis[4] = { pos, pos, pos, pos };

    const int sectDims = sec0 + sec1 + sec2 + sec3;
    const int rotPairs = (sectDims > 0) ? sectDims : halfDim;
    if (i >= rotPairs) {
        return;  // outside the rotary block -> pass through untouched
    }

    float posSel = posAxis[0];
    if (sectDims > 0) {
        const int sector = i;
        if (sector % 3 == 1 && sector < 3 * sec1) {
            posSel = posAxis[1];
        } else if (sector % 3 == 2 && sector < 3 * sec2) {
            posSel = posAxis[2];
        } else if (sector % 3 == 0 && sector < 3 * sec0) {
            posSel = posAxis[0];
        } else {
            posSel = posAxis[3];
        }
    }

    __half* x = x_base + (size_t)startPos * (size_t)writeOffsetStride;
    const int   rotaryDim = 2 * rotPairs;
    const float invDim = 1.0f / (float)rotaryDim;
    const float freq   = powf(base, -(float)(2 * i) * invDim);
    const float theta  = posSel * freq;
    const float c      = cosf(theta);
    const float s      = sinf(theta);

    const int headBase = (p * numHeads + h) * headDim;
    // Rotate in fp32 registers; store back as __half.
    const float a = __half2float(x[headBase + i]);
    const float b = __half2float(x[headBase + i + rotPairs]);
    x[headBase + i]            = __float2half(a * c - b * s);
    x[headBase + i + rotPairs] = __float2half(a * s + b * c);
}
